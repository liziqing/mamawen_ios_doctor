...
#pragma -
#pragma mark 切换扬声器
    [[UIDevice currentDevice] setProximityMonitoringEnabled:YES];  // 开启红外感应？  注意关闭红外感应  -- 省电
    if ([UIDevice currentDevice].proximityMonitoringEnabled == YES) {
        [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(sensorStateChange:) name:UIDeviceProximityStateDidChangeNotification object:nil]; // 监听靠近脸还是远离
    } 
...
#pragma -
#pragma 为何这个设置与否对录音和播放都不影响（overRide控制输出方式） 先用上面这个试试
...
- (void)initRecordView
{
    if (recorderView == nil)
    {
    recorderView = [[ChatRecorderView alloc]initWithFrame:CGRectMake(0, 0, 120, 120)];
    //还原界面显示
...
- (void)updateMeters{
    if (recorder.isRecording){
        ...
        [recorderView updateMetersByAvgPower:lowPassResults];
//        NSLog(@"峰值:%f",[recorder averagePowerForChannel:0]);
        
        //倒计时
        
        if (curCount >= maxRecordTime - 50 && curCount < maxRecordTime) {
            //剩下10秒
...
- (void)touchEnded:(CGPoint)_point{
    //停止计时器
    [self stopTimer];
    [UUProgressHUD dismissWithSuccess:@"Sucess!"];
